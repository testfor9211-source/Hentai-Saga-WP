...
function hentai_saga_customize_register($wp_customize) {
    ...
    // Footer Links
    $footer_links = array('privacy', 'terms', 'dmca', 'usc', 'cookies', 'disclaimer', 'ads', 'age', 'parental');
    foreach ($footer_links as $key) {
        $label_map = array('privacy' => 'Privacy Policy', 'terms' => 'Terms of Service', 'dmca' => 'DMCA Copyright Policy', 'usc' => 'USC 2257', 'cookies' => 'Cookie Policy', 'disclaimer' => 'Disclaimer', 'ads' => 'Advertisement Policy', 'age' => 'Age Restriction Policy', 'parental' => 'Parental Controls');
        $label = $label_map[$key];
        ...
        $wp_customize->add_setting('hentai_saga_footer_' . $key . '_url', array(
            'default' => home_url('/' . str_replace(' ', '-', strtolower($label)) . '/'),
            'sanitize_callback' => 'esc_url_raw'
        ));
...
// Load helper functions from customizer.php
